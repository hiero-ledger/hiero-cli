...
import { ImportTopicCommand } from '@/plugins/topic/commands/import/handler';
...
describe('topic plugin - import command (ADR-007)', () => {
  ...
  test('imports topic successfully with name', async () => {
    ...
    const result = await new ImportTopicCommand().execute(args);
  ...
  test('throws when topic already exists in state', async () => {
    ...
    await expect(new ImportTopicCommand().execute(args)).rejects.toThrow(
