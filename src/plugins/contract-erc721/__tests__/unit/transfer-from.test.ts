...
describe('contract-erc721 plugin - transferFrom command (unit)', () => {
  ...
  test('calls ERC-721 transferFrom successfully and returns expected output', async () => {
    ...
    const result = await contractErc721TransferFrom(args);
  ...
  test('throws TransactionError when execute returns success false', async () => {
    ...
    await expect(contractErc721TransferFrom(args)).rejects.toThrow(
      TransactionError,
    );
    await expect(contractErc721TransferFrom(args)).rejects.toThrow(
  ...
  test('propagates error when alias not found for contract', async () => {
    ...
    await expect(contractErc721TransferFrom(args)).rejects.toThrow('not found');
  });
  ...
  test('throws NotFoundError when resolveAccount for from has no evmAddress', async () => {
    ...
    const promise = contractErc721TransferFrom(args);
