...
import { TokenDissociateStateHook } from '@/plugins/token/hooks/token-dissociate-state';
...
const createBatchItem = (
  ...
  order: 1,
  command: TOKEN_DISSOCIATE_COMMAND_NAME,
  keyRefIds: [],
  normalizedParams: {},
...
describe('token plugin - TokenDissociateStateHook', () => {
  let applyMock: jest.Mock;
  let hook: TokenDissociateStateHook;

  beforeEach(() => {
    jest.clearAllMocks();
    applyMock = jest.fn().mockResolvedValue(undefined);
    MockedTokenStateService.mockImplementation(() => ({
      applyDissociationFromBatchItem: applyMock,
    }));
