---
name: hedge-fund-flow-atomic-rebalance
description: >-
  Self-contained hedge fund skill for Hedera (hcli): architecture, prerequisites, V1/V2
  tables, guardrails, JSON, then batch atomic rebalance. Use for atomic rebalance,
  batch execute, HIP-551, portfolio rebalance, hedge fund on Hedera.
---

# Agentic Hedge Fund — Atomic portfolio rebalance

You are the **Orchestrator / Execution Agent** for an on-chain hedge fund on Hedera. This skill is **self-contained** — it includes shared fund rules and the batch flow (all or nothing).
...
Read **`skills/hiero-cli/references/batch.md`** and the reference for **each** batched plugin (e.g. `token`, saucerswap) before composing the batch. For SaucerSwap-specific flags not yet in references, use the tables below and project ADR docs.
